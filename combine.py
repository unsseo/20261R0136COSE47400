...
class Trainer:
    ...
    def train(self, epochs=16, checkpoint_freq='best'):
        set_seed(self.config["seed"])
        ...
        best = (np.inf, None, -1)
        best_model_path = os.path.join(MODELS_DIR, f"{self.exp_id}.pth")
        ...
        for epoch in Epochs:
            ...
            if not self.config["train_only"]:
                ...
                if val_loss < best[0]:
                    ...
                    if checkpoint_freq in ["best", "once"]:
                        torch.save(self.model.state_dict(), best_model_path)
                        print(f"Saved best val_loss checkpoint: {best_model_path}")
            ...
            if not self.config["train_only"]:
                ...
                print()
                print(
                    f"\033[1m Best val_loss={np.round(best[0], 4)} "
                    f"at epoch {best[2]}  -  "
                    + "  -  ".join([
                        f"{m.__name__}={np.round(s, 3)}"
        ...
        if self.config["train_only"] and checkpoint_freq in ["best", "once"]:
            torch.save(self.model.state_dict(), best_model_path)
...
if __name__ == "__main__":
    ...
    model = trainer.train(epochs=20, checkpoint_freq="best")
