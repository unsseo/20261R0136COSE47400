...
class SpecAugment(nn.Module):
    def __init__(self, time_mask_param=32, freq_mask_param=24, p=0.5):
        super().__init__()
    ...
    def forward(self, x):
        ...
        aug_x = self.freq_mask(x.clone())
        aug_x = self.time_mask(aug_x)

        if self.p >= 1.0:
            return aug_x

        mask = (torch.rand(x.size(0), 1, 1, 1, device=x.device) < self.p)
        return torch.where(mask, aug_x, x)
...
class Trainer:
    ...
    def __init__(self, config={}, fold=0, epochs=16):
        ...
        self.specaug = SpecAugment(**self.config['specaug']).to(self.device) if self.config.get('specaug') else None
...
if __name__ == "__main__":
    ...
    "mix": {"alpha": 0.2, "theta": 0.8},
    "pretrained": True,
    "model": BirdModel,
    'train_only': False,
    'data_path': DATA_PATH,
    'specaug': {"p": 0.3, "freq_mask_param": 12, "time_mask_param": 24, "num_masks": 1}, 
